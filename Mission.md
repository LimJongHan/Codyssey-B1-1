...
다음 **2가지 산출물**을 제출해야 한다.

### 요구사항 수행 내역서 (문서 1개)

- 수행 내역 및 설정/명령어 기록 (SSH 포트, 방화벽 규칙, 계정/그룹/ACL, 디렉토리/권한, 환경 변수, cron 등록 등)
...
#### SSH 설정
- SSH 접속 포트를 **20022**로 변경한다.
- Root 원격 로그인을 차단한다.

> **확인 방법:** sshd 설정 파일에서 포트/PermitRootLogin 확인, `ss -tulnp` 후 sshd 관련 라인 확인

#### 방화벽 설정 (택1)
- UFW 또는 firewalld 중 하나를 선택해 활성화한다.
- 인바운드 허용 포트: **TCP 20022(SSH)**, **TCP 15034(APP)** 만 허용한다.

> **확인 방법:** UFW → `ufw status` / firewalld → `firewall-cmd --list-all`

---

### 4-2. 계정/그룹/권한 체계 (협업 + 최소 권한)
...
| 디렉토리 | 그룹 | 권한 |
|----------|------|------|
| `upload_files` | agent-common | R/W 가능 |
| `api_keys` | agent-core ONLY | R/W 가능 |
| `/var/log/agent-app` | agent-core ONLY | R/W 가능 |

> **확인 방법:** `id agent-admin` / `id agent-dev` / `id agent-test`, `ls -l` 및 `getfacl`로 소유/권한 확인
...
| 변수 | 값 |
|------|----|
| `AGENT_HOME` | `/home/agent-admin/agent-app` (예시) |
| `AGENT_PORT` | `15034` |
...
### 4-4. 시스템 관제 자동화 스크립트 (monitor.sh)
...
- **프로세스:** `agent-app` 실행 상태 확인 → 비정상 시 `exit 1`
- **포트:** TCP 15034 LISTEN 상태 확인 → 비정상 시 `exit 1`

#### 상태 점검 (경고만 출력)
- 방화벽(UFW 또는 firewalld) 활성화 상태 점검
- 비활성 상태면 `[WARNING]` 출력, 스크립트는 종료하지 않음
...
- **로그 포맷:**
  ...
  ```
- **용량 관리:** 최대 10MB / 10개 파일 유지 (logrotate 또는 스크립트 로직 구현)

#### 자동 실행 (cron)
- `agent-admin` 계정의 crontab으로 **매분 실행** 등록
- 등록 후 1~2분 내 `monitor.log`에 새 라인이 자동 누적되는 것을 확인
...
- `monitor.log`를 분석해 CPU/MEM/DISK의 **평균/최대/최소**와 샘플 수를 콘솔로 출력한다.
...
| 단계 | 대상 | 동작 |
|------|------|------|
| 압축 | `/var/log/agent-app/*.log` 중 7일 이상 경과 | 압축 |
| 아카이브 이동 | 압축된 파일 | `/var/log/monitor/agent-app/archive/` 로 이동 |
| 삭제 | `/var/log/monitor/agent-app/archive/*.gz` 중 30일 이상 경과 | 삭제 |

- (권장) 예외 처리 포함: 디렉토리 미존재, 권한 부족, 대상 파일 0개 등에서 안전하게 종료/경고 처리
...
- 제공된 Python 앱은 "실행 대상"이며, 과제의 핵심은 **관제/자동화 스크립트 구현**이다.
