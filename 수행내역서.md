...
**계정 확인 (`id agent-admin && id agent-dev && id agent-test`)**
...
**ACL 확인 (`getfacl $AGENT_HOME/upload_files && getfacl $AGENT_HOME/api_keys && getfacl /var/log/agent-app`)**
...
**확인 (`tail -6 /home/agent-admin/.bashrc`)**
...
![Boot Sequence 및 포트 리슨 확인](./images/08_boot_sequence.png)
...
**확인 (`ls -la $AGENT_HOME/bin/monitor.sh`)**

```
-rwxr-x---  1 agent-dev  agent-core  ...  $AGENT_HOME/bin/monitor.sh
```

![monitor.sh 권한 확인](./images/09_monitor_permission.png)
...
![monitor.sh 수동 실행 결과](./images/10_monitor_run.png)
...
**등록 내용 (`su - agent-admin -c 'crontab -e'` 로 편집 후 저장)**
...
**확인 (`su - agent-admin -c 'crontab -l'`)**

```
* * * * * AGENT_HOME=/home/agent-admin/agent-app AGENT_PORT=15034 AGENT_LOG_DIR=/var/log/agent-app bash /home/agent-admin/agent-app/bin/monitor.sh >> /var/log/agent-app/cron.log 2>&1
```

![crontab 등록 확인](./images/11_crontab.png)

### 5-2. monitor.log 자동 누적 확인

**확인 (`tail -10 /var/log/agent-app/monitor.log`)**
...
![monitor.log 자동 누적 확인](./images/12_monitor_log.png)
...
![report.sh 전체 실행 결과](./images/13_report_full.png)
...
![report.sh 구간 필터 실행 결과](./images/14_report_range.png)
...
![archive.sh 실행 결과](./images/15_archive.png)
